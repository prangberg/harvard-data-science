...
For this project, we create a movie recommendation system using the MovieLens data set. The version of movielens included in the dslabs package is just a small subset of a much larger data set with millions of ratings. We will  create a recommendation system using the 10M version of the MovieLens data set.
...
## 1. Introduction 
that describes the data set and summarizes the goal of the project and key steps that were performed
...
```{r refine RMSE for lambdas}
lambdas <- seq(5.23, 5.25, 0.005) 
lambdas <- seq(5.2, 5.5, 0.01) 
#lambdas <- 5.25  
lambda_rmses <- sapply(lambdas, Movie_User_Lambda)
...
Movies and Users have a very strong impact on the accuracy of the prediction and the RMSE. Year seems to be a bad predictor of a users rating.
