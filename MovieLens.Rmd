...
The genre field can contain multiple values. To analyze it, we need to seperate the string into multiple values. We create a line for each genre listed. To not falsify the overall stats, we save the new data as a seperate variable.
...
Different genres seem to enjoy different popularity with the users. Horror movies averge the lowest rating (3.3). The highest ratings go to IMAX, War, Documentary and Film-Noir (3.8-4). There is a lot of variability in movies that are not assigned to a genre.
